...
# === 10. TEMPORARY LABEL ENCODING FOR IMPUTATION ===
df_encoded = ds.copy()
label_encoders = {}
...
#Income Distribution
sns.histplot(df_final['Income'], kde=True)
plt.title('Income Distribution')
plt.show()

#Categorical vs. Gender (Bar Plots / Percentages)
for col in ['City', 'State', 'Country', 'Month', 'Product_Category', 'Product_Brand', 'Product_Type']:
    pd.crosstab(df_final[col], df_final['Gender'], normalize='index').plot(kind='bar', stacked=True)
...
#Income vs Gender
sns.boxplot(data=df_final, x='Gender', y='Income')
...
# === 18. FINAL LABEL ENCODING FOR MODELING ===
...
# === 19. SAVE FINAL CLEANED DATA ===
#df_final.to_csv("cleaned_data_final.csv", index=False)

# === 20. OPTIONAL: CLEANUP MEMORY ===
gc.collect()
...
# === 27. DECODE MANUALLY ENCODED COLUMNS FOR TABLEAU ===

# Restore original categorical values
income_reverse_map = {1: 'Low', 2: 'Medium', 3: 'High'}
month_reverse_map = {
...
# Decode only if the columns exist
if 'Income' in df_tableau.columns:
    df_tableau['Income'] = df_tableau['Income'].map(income_reverse_map)

if 'Month' in df_tableau.columns:
    df_tableau['Month'] = df_tableau['Month'].map(month_reverse_map)

# Decode Gender if needed
if 'Gender' in df_tableau.columns:
    gender_map = {v: k for k, v in label_encoders_final['Gender'].classes_.items()} if isinstance(label_encoders_final['Gender'], dict) else {0: 'Female', 1: 'Male'}
    df_tableau['Gender'] = df_tableau['Gender'].map(gender_map)

# === 28. SAVE FOR TABLEAU VISUALIZATION ===
tableau_file_path = "cleaned_data_for_tableau.csv"
df_tableau.to_csv(tableau_file_path, index=False)
...
# === IMBALANCE DETECTED - imbalanced dataset, model is likely biased toward 
#predicting the majority class (Gender = 1)

# === modified pipeline to handle class imbalance, using class_weight='balanced'
...
print("LOGISTICAL REGRESSION with modified pipeline to handle class imbalance")
